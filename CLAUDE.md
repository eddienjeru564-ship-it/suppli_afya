...
- Suppli Afya is independent of BF Suma. No BF Suma logos; keep the disclaimer. Product photos only when official and matching the exact pack (see `assets/products/README.md`); never generated, redrawn or approximated.
...
- `src/components/check/`: health check UI (used on the landing page demo, `/check` and `/d/[slug]/check`)
- `src/app/d/[slug]/` + `src/components/shop/`: each distributor's customer shop (home, product pages, basket, checkout, the check). Kate Cromuel (`/d/kate-cromuel`, `src/config/distributors.ts`) is the canonical example; `src/server/shop.ts` loads any shop and places orders
- `src/components/site/`: landing page sections, in story order in `src/app/page.tsx`
- `src/config/`: site settings, plans and prices (`plans.ts`), onboarding options, the demo distributor and shop, product photo list (`product-images.json`, generated by `scripts/product-images.mjs`)
